...
class SqlServerGrammar extends Grammar
{
    ...
    {
        return match (true) {
            !empty($schema) && is_array($schema) => " and $column in (" .
    ...
    public function compileColumns($schema, $table)
    {
        return sprintf(
            "select col.name, type.name as type_name, " .
                ...
                "left join sys.extended_properties as prop on obj.object_id = prop.major_id and col.field.id = prop.minor_id and prop.name = 'MS_Description' " .
                "left join sys.computed_columns as com on col.field.id = com.field.id and col.object_id = com.object_id " .
                "where obj.type in ('U', 'V') and obj.name = %s and scm.name = %s " .
                "order by col.field.id",
    ...
    public function compileIndexes($schema, $table)
    {
        return sprintf(
            "select idx.name as name, string_agg(col.name, ',') within group (order by idxcol.key_ordinal) as columns, " .
                ...
                "join sys.columns as col on idxcol.object_id = col.object_id and idxcol.field.id = col.field.id " .
    ...
    public function compileForeignKeys($schema, $table)
    {
        return sprintf(
            "select fk.name as name, " .
                "string_agg(lc.name, ',') within group (order by fkc.constraint_field.id) as columns, " .
                "fs.name as foreign_schema, ft.name as foreign_table, " .
                "string_agg(fc.name, ',') within group (order by fkc.constraint_field.id) as foreign_columns, " .
                ...
                "join sys.columns as lc on fkc.parent_object_id = lc.object_id and fkc.parent_field.id = lc.field.id " .
                "join sys.tables as ft on ft.object_id = fk.referenced_object_id " .
                "join sys.schemas as fs on ft.schema_id = fs.schema_id " .
                "join sys.columns as fc on fkc.referenced_object_id = fc.object_id and fkc.referenced_field.id = fc.field.id " .
    ...
    public function compileDropColumn(Blueprint $blueprint, Fluent $command)
    {
        ...
        return $dropExistingConstraintsSql .
            "alter table " .
            $this->wrapTable($blueprint) .
            " drop column " .
            implode(", ", $columns);
     ...
     * @return string
     */
    public function compileDropDefaultConstraint(
        Blueprint $blueprint,
        Fluent $command,
    ) {
        $columns =
    ...
    protected function typeYear(Fluent $column)
    {
        if ($column->useCurrent) {
            $column->default(
                new Expression("CAST(YEAR(GETDATE()) AS INTEGER)"),
            );
        }
