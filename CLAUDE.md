...
These rules always apply. Boost owns the tagged block below and rewrites it on every
`boost:update`; everything above that block is ours and stays untouched. Never write the block's
...
Bugs, tasks and ideas go through two-way-ticket's API — see the `magicoli/two-way-ticket` section
in the Boost block below.
...
=== magicoli/two-way-ticket/core rules ===
