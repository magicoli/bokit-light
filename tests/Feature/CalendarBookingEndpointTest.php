...
describe('calendar booking endpoint', function () {
    test('returns booking details with price, paid, balance and panel links', function () {
        ...
        expect($response->json('view_url'))->toContain("/app/{$this->property->slug}/bookings/{$booking->id}");
        expect($response->json('edit_url'))->toContain("/app/{$this->property->slug}/bookings/{$booking->id}");
    });
    ...
    test('hides cancelled bookings from the calendar by default and shows them on demand', function () {
        ...
        $this->get("/app/{$this->property->slug}/calendar?date=".$date->format('Y-m-d'))
            ->assertSuccessful()
            ->assertSee('Confirmée Visible')
            ->assertDontSee('Annulée Visible');

        $this->get("/app/{$this->property->slug}/calendar?date=".$date->format('Y-m-d').'&cancelled=1')
            ->assertSuccessful()
    ...
    test('skips the property header row when a single property is displayed', function () {
        ...
        $this->get("/app/{$this->property->slug}/calendar")
            ->assertSuccessful()
