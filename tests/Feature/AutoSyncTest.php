...
describe('AutoSync', function () {
    test('syncs the units configured sources, with the legacy ical table empty', function () {
    ...
    test('walks every unit when nothing is targeted', function () {
        $called = [];
    ...
    test('resyncs only the booking own unit from the calendar', function () {
        $called = [];
        app(SyncRegistry::class)->register(makeAutoSyncConnector(
            'auto-source',
            [
